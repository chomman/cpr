	...
	<div id="wrapper">
		...
		<div id="right">
			<div id="breadcrumb">
				...
				&raquo; <span><spring:message code="cpr.report.edit" /></span>
			</div>
			<h1>
				<spring:message code="cpr.report.edit" />
			</h1>

			<div id="content">

				<ul class="sub-nav">
					<li><a:adminurl href="/cpr/reports">
							<spring:message code="cpr.report.list" />
						</a:adminurl></li>
					<li><a:adminurl href="/cpr/report/add">
				...
				<form:form commandName="report" method="post" cssClass="valid">
					...
					<p>
						<label> <spring:message code="cpr.report.dateFrom" />:
						</label> <span class="field"> <form:input path="dateFrom"
								maxlength="25" cssClass="date required" /> <span> <spring:message
									code="cpr.report.dateTo" />:
						</span> <form:input path="dateTo" maxlength="25" cssClass="date required" />
						</span>
					</p>
					<p>
						<label title="<spring:message code="publish.descr" />" class="tt">
							<spring:message code="publish" />
						</label> <span class="field"> <form:checkbox path="enabled" />
						</span>
					</p>
					<p>
					...
					<p class="switchable cs">
						<label> <spring:message code="cpr.report.content" /> (<spring:message
								code="cs" />)
						</label> <span class="field"> <form:textarea path="contentCzech"
								cssClass="wisiwig" />
						</span>
					</p>
					<p class="switchable en hidden">
						<label> <spring:message code="cpr.report.content" /> (<spring:message
								code="en" />)
						</label> <span class="field"> <form:textarea path="contentEnglish"
								cssClass="wisiwig" />
						</span>
					</p>
					 <form:hidden path="id"/> 
					<p class="button-box">
						<input type="submit" class="button default"
							...
							<a href="" data-url="report/${report.id}?isPreview=true" class="button preview" data-fancybox-type="iframe"><spring:message code="form.preview" /></a>
					</p>
				</form:form>
					...
					<c:if test="${not empty model.standardCsns}">
						<table class="csn">
							<c:forEach items="${model.standardCsns}" var="csn">
								<tr>
									<td class="name key"><a:standardCsnUrl object="${csn}"
											editable="${editable}" /></td>
									<td><c:if
											test="${not empty csn.standardStatus and csn.standardStatus.id != 1}">
								...
								<c:forEach items="${csn.standardCsnChanges}" var="j">
									<tr class="standard-change csn-change">
										<td class="name key"><a:standardCsnUrl object="${j}"
												editable="${editable}" /></td>
										<td>${csn.note}</td>
									</tr>
								</c:forEach>
