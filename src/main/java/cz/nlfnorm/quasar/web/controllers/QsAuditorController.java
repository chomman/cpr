...
public class QsAuditorController extends QuasarSupportController{
	...
	private void evaluateQsAuditor(ModelMap modelMap, final Auditor auditor){
		final QsAuditor qsAuditor = qsAuditorAssesmentService.evaluateFor(auditor);
		prepareModel(modelMap, qsAuditor, auditor);
	}
	
	private void prepareModel(ModelMap map, AuditorFunction object, Auditor auditor){
		Map<String, Object> model = new HashMap<>();
		model.put("auditor", auditor);
		model.put("evaludatedAuditor", object);
		//appendSubTab(model, SUB_TAB_QS_ADUTITOR);
